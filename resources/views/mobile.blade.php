<div id="mobile-intro" class="d-md-none">
    <div id="stoplight">
        <div class="row">
            <div class="col stoplight-col">
                <div class="green-light light-circle"></div>
            </div>
        </div>
    </div>

    <div class="d-flex justify-content-center">
        <img src="/images/kings-park-dance-logo.png" alt="" style="width: 200px; height: 157px;">
    </div>
    <div class="row mt-4 mx-0">
        <div class="col p-0">
            <img src="/images/kings-park-dance-head-2.jpg" alt="" class="img-fluid">
        </div>
    </div>

    <div id="carouselTestimonials" class="carousel slide mt-2" data-ride="carousel" style="font-family: 'Courgette', cursive; font-size: 1.5em;">
        <div class="carousel-inner text-center">
            <div class="carousel-item active">
                <p style="line-height: 1;">Our happy place
                    <br><span class="quotee">- Laura D</span>
                </p>
            </div>
            <div class="carousel-item">
                <p style="line-height: 1;">Boys dance too!
                    <br><span class="quotee">- Brandon N</span>
                </p>
            </div>
            <div class="carousel-item">
                <p style="line-height: 1;">The place to be
                    <br><span class="quotee">- Chris G</span>
                </p>
            </div>
            <div class="carousel-item">
                <p style="line-height: 1;">Very nurturing
                    <br><span class="quotee">- Zoie B</span>
                </p>
            </div>
            <div class="carousel-item">
                <p style="line-height: 1;">Our dance family
                    <br><span class="quotee">- The Hollands</span>
                </p>
            </div>
            <div class="carousel-item">
                <p style="line-height: 1;">Quality and fun
                    <br><span class="quotee">- Kerri A</span>
                </p>
            </div>
            <div class="carousel-item">
                <p style="line-height: 1;">Our second family
                    <br><span class="quotee">- Jenn Z</span>
                </p>
            </div>
        </div>
    </div>
</div>
